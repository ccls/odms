module ActionControllerExtension::Routing
	...
	module ClassMethods
		...
		def assert_no_route(verb,action,args={})
			...
			#	This is the ORIGINAL version.  It worked in Rails 2 and 
			#	then not in Rails 3 with the asset pipeline enabled.
			#	It appears that journey 1.0.4 or ruby 1.9.3 will now raise
			#	an error here!  Yay!  Simplicity!
			test "#{brand}no route to #{verb} #{action} #{args.inspect} SIMPLE" do
					assert_raise(ActionController::RoutingError){
						send(verb,action,args)
					}
			end

			#	replacement (without all the comments)
			#	Once confirmed original works again, can comment this out, but maybe shouldn't
			test "#{brand}no route to #{verb} #{action} #{args.inspect} ASSETS" do
...
##
#assert_raise(Journey::Router::RoutingError){
##	is it the same? NOPE, but the original version works again!!!
##	may be able to simplify all this again!
##assert_raise(ActionController::RoutingError){
#						path, params = Odms::Application.routes.formatter.generate(
#							:path_info,nil,{ :method => verb, :action => action }.merge(args))
#
