...
class ScreeningDatumUpdatesControllerTest < ActionController::TestCase
	...
	ASSERT_ACCESS_OPTIONS = {
		:model => 'ScreeningDatumUpdate',
		:actions => [:new,:create,:show,:destroy,:index],
		:attributes_for_create => :factory_attributes,
		:method_for_create => :create_screening_datum_update
	}
	#	ScreeningDatumUpdate have no attributes other than the csv_file
	#	so need to add the updated_at to force a difference
	#	on update.
	def factory_attributes(options={})
		Factory.attributes_for(:screening_datum_update,{
...
#			assert_difference('CandidateControl.count',0) {
			...
			assert_not_nil assigns(:screening_datum_update).csv_file_file_name
			assert_not_nil assigns(:screening_datum_update).csv_file_content_type
			assert_not_nil assigns(:screening_datum_update).csv_file_file_size
			assert_not_nil assigns(:screening_datum_update).csv_file_updated_at
		end
...
#			assert_difference('CandidateControl.count',1) {	#	may depend on record content!
			...
			assert_difference('ScreeningDatumUpdate.count',1) {
				post :create, :screening_datum_update => Factory.attributes_for(
					:one_record_screening_datum_update )
			} } #}
			assert_not_nil flash[:notice]
			assert_not_nil assigns(:screening_datum_update).csv_file_file_name
			assert_not_nil assigns(:screening_datum_update).csv_file_content_type
...
#			assert_difference('ScreeningDatumUpdate.count',0) {
#				put :update, :id => screening_datum_update.id, 
#					:screening_datum_update => factory_attributes
#			}
#			screening_datum_update.reload
#			assert_not_nil flash[:notice]
#			assert_not_nil screening_datum_update.csv_file_file_name
#			assert File.exists?(screening_datum_update.csv_file.path)
...
#	should I allow destroying?
		...
		test "should destroy with csv_file attachment and #{cu} login" do
			...
			screening_datum_update = Factory(:screening_datum_update)
			assert_difference('ScreeningDatumUpdate.count',-1) {
				delete :destroy, :id => screening_datum_update.id
			}
		end
...
#			login_as send(cu)
#			create_case_for_screening_datum_update
#			screening_datum_update = create_test_file_and_screening_datum_update
#			assert_difference('CandidateControl.count',1){
#				post :parse, :id => screening_datum_update.id
#			}
...
#			login_as send(cu)
#			screening_datum_update = Factory(:screening_datum_update)
#			File.delete(screening_datum_update.csv_file.path)
#			assert_difference('CandidateControl.count',0){
#				post :parse, :id => screening_datum_update.id
#			}
#			assert_not_nil flash[:error]
#			assert_redirected_to assigns(:screening_datum_update)
#		end

#		test "should parse with #{cu} login and real csv_file" do
#			#	real data and won't be in repository
#			real_data_file = "screening_data/screening_datum_update_20120424.csv"
...
#	
#			assert_difference('ScreeningDatum.count',33){
...
#
##			screening_datum_update = Factory(:screening_datum_update,
##				:csv_file => File.open('test-livescreeningdata_011912.csv') )
##			assert_not_nil screening_datum_update.csv_file_file_name
##	
