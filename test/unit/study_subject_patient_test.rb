...
#	end
	...
	test "should create patient for case study_subject" do
		...
		assert_difference( "StudySubject.count", 1 ) {
			...
			assert study_subject.persisted?, 
				"#{study_subject.errors.full_messages.to_sentence}"
			patient = FactoryBot.create(:patient, :study_subject => study_subject)
	...
	test "should NOT create patient for non-case study_subject" do
		...
		assert_difference( "StudySubject.count", 1 ) {
			...
			assert study_subject.persisted?, 
				"#{study_subject.errors.full_messages.to_sentence}"
			patient = FactoryBot.build(:patient, :study_subject => study_subject)
	...
	test "should NOT create study_subject with empty patient" do
		...
		assert_difference( "StudySubject.count", 0 ) {
			...
			assert study_subject.errors.include?('patient.diagnosis'.to_sym)
			assert study_subject.errors.matching?('patient.diagnosis',"can't be blank")
...
#			:matchingid => '012345',	#	seriously? set the subjecid
			:subjectid => '012345',
			:patient_attributes    => FactoryBot.attributes_for(:patient)).reload
...
##		study_subject = create_study_subject( 
##			:identifier_attributes => FactoryBot.attributes_for(:identifier,
