...
Added a "sleep" after starting sunspot as it just seems to take a moment.
 ...
 c ; bundle exec rake sunspot:solr:stop RAILS_ENV=test ; /bin/rm -rf coverage* ; bundle exec rake sunspot:solr:start RAILS_ENV=test ; sleep 120 ; bundle exec rake ; mv coverage coverage-one-run ; open coverage-one-run/index.html ; bundle exec autotest -v --no-full-after-failed



DO NOT KEEP OLD FILES IN DB/MIGRATE/.  They will be included when the test db is migrated.




---
== FULL PRODUCTION GEM REINSTALLATION
