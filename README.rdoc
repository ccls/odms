= CCLS Operations and Data Management System
 ...
 #	we no longer use mail, but you may have to copy
 #	in some mail credentials ...
 cp config/initializers/mail.rb.example config/initializers/mail.rb


 #	Assuming that ruby 1.8.7 is installed.
 #	Of course, some version of rubygems will need to be installed
 #	as well, otherwise there is no 'gem' command.
 #	Should probably update to the latest rubygems (1.8.21 as of 20120404)
 gem update --system
...
CANNOT have leading 0's and include and 8 or 9 in a string that is being printed as a decimal as ruby thinks its octal and 8 and 9 are invalid octal characters so convert back to Integer first.
...
I have now replaced these sleep commands with more logical
'wait_until' blocks which seems to work just fine.  In addition,
I upped the default time out.  Now they rarely fail, but when
they do, it is usually due to a timeout.
...
== Additional files that will be needed
 ...
 /config/database.yml
 /config/initializers/mail.rb	(not doing mail anymore so irrelevant)
