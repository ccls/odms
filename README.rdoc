...
* add User.with_role_names tests

* update subject race and language selectors

* investigate candidate control integration test failures
...
* fix user not logged in integration test.  current_url doesn't seem to match followed redirect.
...
*	get acts_as_list working (actually seems to work fine.  tests don't though)
  ...
  * default_scopes seem to be the problem.  Don't use them.
